...
for fp in sorted(PROC.rglob("*.parquet")):
    ...
    try:
        ...
        if nrows == 0:
            ...
            continue
        # Read data_role metadata once (used in multiple branches below)
        ...
        if not isinstance(idx, pd.DatetimeIndex):
            # Auxiliary, event catalog, multi-station, and multi-dimension long
            # datasets legitimately use RangeIndex or MultiIndex
            _ok_roles = ("auxiliary", "event_catalog", "multi_station",
                         "multi_dimension_long")
            if any(r in _role for r in _ok_roles):
                ok(name, f"{nrows:,}r x {ncols}c | {_role} -- non-DatetimeIndex expected")
            else:
        ...
        if idx.tz is None:
            ...
            if dups > 0:
                # Event catalogs and multi-station files legitimately have
                # multiple rows per timestamp -- treat as OK if tagged
                _dup_ok_roles = ("event_catalog", "multi_station")
                if any(r in _role for r in _dup_ok_roles):
                    ok(name, msg + f" | {dups} duplicate timestamps expected ({_role})")
                else:
...
for fp in sorted(PROC.rglob("*.nc")):
    ...
    try:
        ...
        # Required coordinate variables (check both variable names and dimension names)
        # lon/longitude is optional for zonal-mean and polar products
        varnames = list(ds.variables.keys())
        dimnames = list(ds.dimensions.keys())
        all_names = set(varnames) | set(dimnames)
        has_time = any(v in all_names for v in ("time", "valid_time", "t"))
        has_lat  = any(v in all_names for v in ("lat", "latitude"))
        has_lon  = any(v in all_names for v in ("lon", "longitude"))
        is_zonal = (not has_lon) and any(
            v in str(getattr(ds, "title", "")).lower() + fp.name.lower()
            for v in ("zonal", "mls", "polar", "mean", "strat", "grid"))
...
range_checks = [
    ...
    ("solar/omni_1min.parquet",          "Vx",             -1200, -90,  "km/s"),   # Vx GSE (negative = sunward)
    ("solar/omni_1min.parquet",          "Bz",            -90,   90,    "nT"),    # Bz GSE
    ("solar/omni_1min.parquet",          "B_magnitude",    0,    100,   "nT"),    # |B| total
    ("solar/omni_1min.parquet",          "flow_speed",     100, 1200,   "km/s"),  # scalar speed
    ("solar/omni_1min.parquet",          "proton_density", 0,     90,   "cm-3"),  # n_p
    ...
    # NCEP stratosphere: polar cap air temperature must be non-zero
    ("atmospheric/ncep_stratosphere.parquet", "air_K_10hPa",  170, 280, "K"),
    ("atmospheric/ncep_stratosphere.parquet", "air_K_50hPa",  175, 280, "K"),
    # NCEP troposphere
    ("atmospheric/ncep_troposphere.parquet",  "hgt_500hPa_m", 5000, 6000, "gpm"),
    ("atmospheric/ncep_troposphere.parquet",  "slp_Pa",       98000, 104000, "Pa"),
]
...
required = {
    ...
    "atmospheric/ncep_stratosphere.parquet":     "NCEP polar strat temperature (polar vortex / SSW)",
    "atmospheric/ncep_troposphere.parquet":      "NCEP troposphere (surface forcing bridge)",
    # Surface forcing
...
for rel, desc in required.items():
    ...
    if fp.exists():
        ...
        ok(f"REQUIRED: {fp.name}", f"{desc} -- {sz:.1f} MB [OK]")
    else:
